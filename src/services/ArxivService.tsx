...
class ArxivService {
    public async queryArxiv(terms: string[]): Promise<Paper[]> {
        var query = terms.join("+OR+").replaceAll(" ", "\\ ");
        console.log(`http://export.arxiv.org/api/query?search_query=all:${query}`);
                        ...
                        return (
                            {
                                ...
                                abstract: entry.filter((val: any) => val.name === "summary")[0].value
                            }
                        )
                    })
                    return papers;
